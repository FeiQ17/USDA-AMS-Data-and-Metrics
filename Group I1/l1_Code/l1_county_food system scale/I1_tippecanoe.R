...
colors <- c("#00008B", "#006400", "#FFAE00", "#E26A6A")  # Blue, Green, Orange, Red
...
p <- ggplot() +
  ...
  scale_fill_manual(values = colors, 
                    guide = guide_legend(title = "Categories", 
                                         override.aes = list(alpha = c(0.8, 0.6, 0.4, 0.2), size = 5)),
                    labels = labels) +   # Specify the desired legend labels
  geom_text(aes(x = -side_length/2 - 1.8, y = side_length/2, label = labels[1]), hjust = 0, vjust = 1) +
  geom_text(aes(x = side_length/2 + 2, y = side_length/2, label = labels[2]), hjust = 1, vjust = 1) +
  ...
  ggtitle("Strength of the Food System in Tippecanoe County") +   # Updated title
